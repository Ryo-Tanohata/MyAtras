...
- 2026-09-16 06:00 〜 09-19 04:00 UTCの71枚（1時間ごと、3日分、13.9MB）の全球赤外線観測を順に再生。`scripts/fetch-observations.cjs --span 72 --frames 72 --every 60 --winds` で取得し同梱にした（72時間でSSECが列挙したのは71時刻）。各時刻の風3.4MB・観測間の動き1.2MBも同梱。以前の同梱は履歴にある。再生の速さは「1秒で何時間進むか」（1.5 / 4.5 / 9、既定4.5）で、観測の間隔が変わっても見え方が変わらない。両ページとも観測を並列で読み込む（JS版6本、Unity版8本同時）。下り10 Mbps・遅延80 ms・キャッシュなしで再生開始まで JS版15.2秒（1つずつなら22.7秒）、Unity版26.3秒（同53.2秒）。日時は日本時間で表示。データは保存時点の観測であり常時最新ではない。
