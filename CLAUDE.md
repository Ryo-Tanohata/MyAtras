...
2026-09-19、ユーザーの希望で「それっぽく見える」模型の方向へ振った。Unity版の既定は「風で流す模型」で、観測と観測の間を、前の観測を風で前へ・次の観測を風で後ろへ動かして重ねる動き補償の補間でつなぐ（シェーダーの `Pair`、`ObservationPlayback.cs` の `Model`）。風は各観測時刻のAMV（`dist/data/wind/`、前後2時刻の平均）で、観測点の近くはその値、9°以内はガウス重みで広げ、それも届かない所だけ典型的な大気循環（貿易風・偏西風・極東風、東西成分のみ）。観測ごとに必ず実観測に戻るので雲の配置は観測から離れない。間の動きは推定で観測ではないとページに常に明記し、「実観測だけ」に切り替えられる。一時停止はその場で止まる。
ユーザーは「雲が少し動いてまた同じ繰り返し」を気にしていた。原因は2つ：24時間のループの短さと、風が実際の雲の動きと合わない所で毎回観測へ引き戻されること。前者は3日分にして解消。後者は測定した：3時間の各区間で前の観測を風で流して次の観測と比べると、ずれは動かさない場合に比べて 典型的な循環だけ−12.8%、1時刻の風−14.2%、各時刻の風−16.2%。10°の区画ごとに最良の平行移動を当てはめた上限でも−31.7%で、3時間の変化の約7割は雲の発生・消滅・変形なので、流すだけでは揺り戻しはゼロにならない。画像から動きを推定すれば−16%から上限の−30%近くまでは改善の余地がある。
