...
- 2026-09-16 06:00 〜 09-19 03:00 UTCの24枚（3時間ごと、3日分）の全球赤外線観測を順に再生。`scripts/fetch-observations.cjs --span 72 --frames 24 --every 180 --winds` で取得し同梱にした。1時間ごと1日分と同じ容量で、ループが長く、1コマの動きが大きい。以前の同梱は履歴にある。日時は日本時間で表示。データは保存時点の観測であり常時最新ではない。
