...
JS版に実装（`dist/storms.js` の `StormOutlook`、スイッチ「この先の見込みを重ねる（過去の台風の平均）」）。**観測が尽きた最後の時刻でのみ**48時間先まで描き、観測のある時刻には重ねない。色も琥珀（観測由来）と紫（見込み）で分ける。147 KBは必要になったときだけ読む。これはこのアプリで初めての「観測より先」の表示なので、予報ではないことを画面に常時明記している。Unity版は未対応。
