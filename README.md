...
71 distinct infrared observations an hour apart, from 2026-09-16 06:00 UTC to 2026-09-19 04:00 UTC - three days - are played in order. They were fetched with `node scripts/fetch-observations.cjs --span 72 --frames 72 --every 60 --winds` (SSEC listed 71 hourly times in the 72 hours) and committed as the bundled set, 13.9 MB, with the wind at each time (3.4 MB) and the motion measured between each pair (1.2 MB); earlier bundled sets are in the history. In the Unity build the model loops the newest 48 hours without a cut, handing the end over to the start by overlaying the two - each still moving forward in time - across 12 hours, at the same hour of the day so the terminator stays put; see `unity/README.md`. The playback speed is hours of the atmosphere per second - 1.5, 4.5 or 9 - so the same setting looks the same whatever the spacing of the observations, and both pages load the observations several at a time: on a 10 Mbit/s connection with 80 ms latency and nothing cached, the JavaScript page starts playing after 15.2 s instead of 22.7 s one file after another, the Unity page after 26.3 s instead of 53.2 s. Each downloaded frame was verified against the SSEC response `RE-Time` header, and the timestamp shown changes with the actual frame. Images/logos are retained. No translation of a fixed image or globe rotation is used to fake the cloud movement. The loop pauses briefly at the last frame then restarts, and supports pause, speed selection and individual observation-time selection. 「読み込む観測の数」 chooses how many observations a refresh through 「最新を取得」 plays - 24, a day, or 12 for a lighter load. SSEC lists the last 24 hours, an hour apart, so 24 is the most a refresh can meet. A choice is met only by times the server actually listed; nothing is invented to fill it, and what was fetched is what plays. New available data can be fetched by pressing 「最新を取得」, and only then: the page never asks SSEC by itself, so visitors put no load on it (see `DATA_POLICY.md`); playback reuses cached frames. Infrared observations are grayscale and include cold surface signatures, not only clouds. This does not reproduce Miraikan’s full-color composite processing.
...
- `node scripts/find-storms.cjs` finds the tropical cyclones in the bundled observations, from the observations alone (no network), into `dist/data/storms.json`. A typhoon is about seven pixels across in a 512-pixel globe, too few to read a spiral from, and a compact mass of deep cold cloud is the commonest thing in the tropics, so a single frame cannot say: what separates a cyclone is that it holds together for days and travels, while convection over land comes and goes in the same place and a cluster along the ITCZ lasts hours. Connected cold areas therefore only seed a system, and a system once seeded is followed by the cold core nearest where it was heading, weighted towards that point - which is what keeps the centre on the storm when its canopy merges into a frontal band. A track is kept when it lasts, goes missing rarely, runs straight, moves at a cyclone's pace and steadily, drifts polewards, and stays clear of the equator, where the Coriolis force is too weak for one to form. The publish workflow rebuilds it whenever it fetches observations.
- Accuracy of `find-storms`, measured against the Japan Meteorological Agency's own analysed track for typhoon 25 (Dujuan) of 2026 - fetched to check this, neither bundled nor depended on: over the bundled three days the centre came back to a median 74 km, about one pixel, with all 68 hours inside 200 km and no false storm anywhere on the globe. The same storm three days earlier, while it was still a developing depression, came back to a median 596 km, because wind shear sets the deep convection away from the circulation and this follows the convection. Nothing in the output tells those two cases apart: each point records how disc-like its core is, but the measure barely moves between them, so it is a hint and not a confidence. The thresholds were chosen against those two windows and no others, and no other storm has been tried.
- `dist/storms.js` reads `dist/data/storms.json` and draws what it holds: an amber ring on the estimated centre for the observation on screen, and a dotted trail for where the storm has already been, in the globe's own camera so a mark lands on the place in the picture it was measured from. It draws only for an observation the storm was actually found in - no centre is interpolated into the hour between two of them, and none is carried past the last one - and the three bundled observations with no detection are left bare and said to be. The switch is labelled with what the mark is, the position and "not an announcement and not a forecast" sit beside it, and the rest is folded into the page's notes.
- `node scripts/build-tendency.cjs --tracks FILE` reads IBTrACS - the archive that gathers every agency's best tracks - and writes `dist/data/tendency.json`: for each 2.5-degree cell, the median heading and pace of the tropical-stage legs that passed through it, and how widely they differed. The archive is 137 MB and is not bundled; the grid is 147 KB and is. What comes out is the general circulation, measured rather than drawn: west-north-west under the subtropical ridge, north as storms reach its western edge, then north-east and accelerating once the westerlies have them. It changes only as decades pass, so it is run by hand and committed, not rebuilt by the publish workflow.
- The spread is the point of it. Over the deep tropics four storms in five ran within 45 degrees of the same heading; through the turn near Japan, barely half did. Whether a storm recurves before Japan or carries on west is genuinely not settled by where it is, and the grid records that so the drawing can show it. Carried forward from the JMA's analysed position for typhoon 25 of 2026 the middle of the grid landed 126 km from their 12-hour forecast and 109 km from their 45-hour one, against forecast circles of 45 km and 155 km: close enough to be worth drawing, far enough to be worth labelling.
- `node tests/build-tendency.test.cjs` checks the reading against a small made-up track file - a known heading and pace, the extratropical stage left out, pre-satellite seasons left out, gaps not crossed, a stalled storm saying nothing, headings either side of north averaging to north - and holds the bundled grid to the circulation it claims to have measured, including that the turn is recorded as less settled than the tropics.
- Observed and expected are told apart by four things at once, not by colour alone: colour reads as importance rather than as the difference between something seen and something guessed, and some readers cannot separate two hues at all. What was observed is warm, continuous, filled and a single line; what is only expected is cool, broken every six hours, hollow, and wrapped in a translucent fan. The fan is the strongest of the four - an observation has no width, so a width means a range of possibilities - and the amber ring is the handover between them.
- The Unity version draws the same two files the same way (`unity/MyAtras/Assets/Scripts/StormMarks.cs`). Its globe is a full-screen shader with no sphere to hang anything on, so the marks are built as screen-space quads after the blit, using the shader's own camera arithmetic so a centre lands where it was measured. A shader nothing references through a material asset is stripped from a WebGL build, so `WebGlBuild.cs` names every shader the build must keep.
- `node tests/storms.test.cjs` covers what the page may draw: a centre for an observation the storm was found in and for no other time, a trail holding only where it has been, gaps and single-point tracks left unmarked, two storms coming back as two marks, and a bundle that will not load leaving the globe bare instead of throwing.
- `node tests/find-storms.test.cjs` draws synthetic storms in the observations' layout and checks that a compact mass travelling steadily is found where it was put and followed the whole way; that convection sitting still, a burst lasting a few hours, and cloud on the equator are each not called a cyclone; that two storms at once come back as two tracks; that a storm whose canopy joins a long front is still followed, which is what the second stage exists for; and that the bundled storms name the bundled observations.
