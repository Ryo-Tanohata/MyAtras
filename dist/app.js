...
document.querySelector('#smoothObservations').onchange=e=>window.geoFade.setEnabled(e.target.checked);document.querySelector('#showStorms').onchange=e=>{showStorms=e.target.checked;showStormStatus();};document.querySelector('#showOutlook').onchange=e=>{showOutlook=e.target.checked;if(showOutlook){const box=document.querySelector('#showStorms');if(!box.checked){box.checked=true;showStorms=true;}if(window.geoOutlook)window.geoOutlook.load().then(()=>showStormStatus());}showStormStatus();};document.querySelector('#panels').onchange=e=>panels=e.target.checked;document.querySelector('#rotate').onchange=e=>rotating=e.target.checked;document.querySelector('#speed').oninput=e=>{speed=Number(e.target.value);document.querySelector('#speedValue').value=speed.toFixed(1)+'×';};document.querySelector('#in').onclick=()=>setZoom(zoom+.12);document.querySelector('#out').onclick=()=>setZoom(zoom-.12);document.querySelector('#reset').onclick=()=>{yaw=2.35;pitch=.20;zoom=1;};document.querySelectorAll('[data-mode]').forEach(b=>b.onclick=()=>{mode=Number(b.dataset.mode);document.querySelectorAll('[data-mode]').forEach(x=>{x.classList.toggle('selected',x===b);x.setAttribute('aria-pressed',String(x===b));});document.querySelector('#modeDescription').textContent=mode?'太陽光を想定した照明で、昼夜の境界を眺める。':'地球全体を照らす、発光する球体ディスプレイ。';});const full=document.querySelector('#fullscreen');if(!document.fullscreenEnabled)full.hidden=true;full.onclick=async()=>{try{if(document.fullscreenElement)await document.exitFullscreen();else await document.documentElement.requestFullscreen();}catch{full.textContent='全画面は利用できません';}};document.addEventListener('fullscreenchange',()=>full.textContent=document.fullscreenElement?'全画面を終了 ↙':'全画面 ↗');
