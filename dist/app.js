...
// How far the dissolve from the simulated hour before to the one on screen has run: the
// same dissolve, over the same time, as between two observations - the scene is shown an
// hour at a time, not moved on every frame. Its violet tint comes in over its first hour.
function simStep(t){if(!fade.enabled)return 1;const ms=Math.max(120,Math.min(380,playback.interval*.55));const k=(t-stormSim.shownAt)/ms;return Math.min(1,Math.max(0,k));}
window.geoSimFade=()=>simStep(performance.now());
