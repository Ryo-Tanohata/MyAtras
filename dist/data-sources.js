...
(function(root){
const SNAPSHOT='weather/snapshot.json',SEQUENCE='weather/sequence/manifest.json',REGION='weather/region/manifest.json',AMV='data/amv.json',STORMS='data/storms.json',TENDENCY='data/tendency.json';
...
const GeoData={
 ...
 // so this simply finds nothing there and the globe stays on the global frames.
 async region(){if(root.GEO_REGION)return root.GEO_REGION;if(root.GEO_WEATHER_SEQUENCE)return null;try{const data=await loadJSON(REGION);if(!data||!Array.isArray(data.frames))return null;const out={...data,frames:data.frames.map(f=>({...f,url:beside(REGION,f.file)}))};root.GEO_REGION=out;return out;}catch(error){return null;}},
